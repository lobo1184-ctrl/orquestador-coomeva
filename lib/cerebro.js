...
const HERRAMIENTAS = [
  ...
  { type: "function", name: "crear_solicitud", description: "Envía la solicitud a la consola de diseño. Devuelve un id para consultar el avance. Solo cuando la persona haya confirmado el resumen.",
    parameters: { type: "object", required: ["solicitud"], additionalProperties: false,
      properties: {
        solicitud: {
          type: "object",
          description: "La solicitud completa. 'contenido' lleva los textos directamente como claves (titular, subtitular, cta, legal), NO anidados dentro de 'textos'.",
          required: ["empresa", "plantilla", "contenido"],
          additionalProperties: false,
          properties: {
            empresa: {
              type: "string",
              description: "Clave de la empresa del catálogo. Ej: ejemplo"
            },
            plantilla: {
              type: "string",
              description: "Clave de la plantilla del catálogo. Ej: post_promo"
            },
            formatos: {
              type: "array",
              description: "Ids de formato a producir. Si se omite, se producen todos.",
              items: { type: "string" }
            },
            contenido: {
              type: "object",
              description: "Textos de la pieza. Las claves son DIRECTAMENTE los campos de la plantilla: titular, subtitular, cta, legal. Ejemplo correcto: {\"titular\":\"20% en plan familiar\",\"subtitular\":\"Hasta el 30 de septiembre\",\"cta\":\"Conoce el plan\",\"legal\":\"Aplican TyC\"}. NUNCA anides dentro de 'textos' ni envíes como string.",
              additionalProperties: { type: ["string", "null"] }
            },
            imagen: {
              type: "object",
              description: "Fotografía principal de la pieza.",
              properties: {
                descripcion: {
                  type: "string",
                  description: "Qué debe mostrar la foto: sujeto, acción, lugar, luz, emoción. Sin texto ni logos."
                },
                politica: {
                  type: "string",
                  enum: ["solo_banco", "banco_luego_generar", "solo_generar", "ninguna"],
                  description: "solo_banco: nunca generar. banco_luego_generar (defecto): buscar y si no hay, generar. solo_generar: no buscar. ninguna: pieza sin foto."
                },
                asset_id: {
                  type: ["string", "null"],
                  description: "Id de una foto concreta del banco (img_…) o adjuntada por la persona (sub_…). Tiene prioridad sobre la descripción."
                },
                evitar: {
                  type: "string",
                  description: "Lo que no debe aparecer en la foto."
                }
              }
            },
            salida: {
              type: "object",
              properties: {
                formato: { type: "string", enum: ["png", "jpg"] },
                escala: { type: "number" }
              }
            },
            notas: {
              type: "string",
              description: "Contexto libre para el diseñador que revise la pieza."
            }
          }
        }
      }
